...
Try {
	...
	[string]$appName = 'Application Uninstaller'
	[string]$appVersion = ''
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '07/11/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$applicationList = 'Adobe','Adobe General Labs Bundle','Photoshop','InDesign','Illustrator','XD','Acrobat','CreativeCloud','Creative Cloud'
		...
		$exitCode = Execute-Process -Path "$dirFiles\AdobeCreativeCloudCleanerTool.exe" -Parameters "--removeAll=ALL" -WindowStyle "Hidden" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		## This next part is for SCCM. We're just creating a directory that we can use for detection and delete when we uninstall.
		Write-Log -Message "Checking to see if directory exists..." -Source 'Pre-Installation' -LogType 'CMTrace'
		If (-not (Test-Path "$envProgramFiles\Adobe\Adobe-Uninstaller-Temp-62CCA825")) {
			Write-Log -Message "Directory does not exist...Creating new directory..." -Source 'Pre-Installation' -LogType 'CMTrace'
			New-Item -Path "$envProgramFiles\Adobe\Adobe-Uninstaller-Temp-62CCA825" -ItemType "directory"
		}

		## Display a message at the end of the install
		If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$appName $appVersion has been run successfully. You can run this as many times as you need simply by uninstalling and reinstalling." -ButtonRightText 'OK' -Icon Information -NoWait }
	}
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		## Delete the directory we created for detection in SCCM.
		If (Test-Path "$envProgramFiles\Adobe\Adobe-Uninstaller-Temp-62CCA825") {
			Get-ChildItem -Path "$envProgramFiles\Adobe\Adobe-Uninstaller-Temp-62CCA825" -Recurse | Remove-Item -force -recurse
			Remove-Item "$envProgramFiles\Adobe\Adobe-Uninstaller-Temp-62CCA825" -Force
		}
		Else {
			Write-Log -Message "Directory not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUKFO57xSNLwGxC8lVMCmZdJT6
# Dx6gghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFBSTCGqjtlQS0XKFg8lD8Y7Z/stBMA0GCSqGSIb3DQEBAQUABIIBgCNPncO2
# jgPViIiMOp+BScZfoLZvB49sET/BBL9N5RC9egW8ocmlEpaX7sSfAWaDUgZjMEjU
# LkQpZ2YVdMPSzZEeyjYMJaLdn5aKPeS4ylZwa3ACWpLOdZavOWNrmupBt3ohLpoW
# 6vJEQqjNbEJPlV8EWC2vQCCTaEK+wTwYgfee2PII3N0CTBp284Q3AnOfWw0V0uP2
# KnNoryIGqdMhb08GIDJabE+AZs1EFfanknck3hTGkFgS/E/8FMkzXWP3cKY/52hc
# UIzbyckPPa2kniyYMMxA8yDRNfbZ48xEcKlKXfYUOLnGsWVJujfmNC2bg6CzG0Jq
# C2QxlVa80MsxyEWOcQoewNxbiQKihr8YsvAeGb5i1oOWUt4WCPhG5RWOQEVn/7r7
# X1dFfrsQAVitDMRHHSDrlNEHY48rIHLwgJVAsHpP7apqnKZyBIn5mjUkCMSqkTYn
# kKM5JFd+Q2UJ1nQ1o0jmDHc1vQZ6DUSJ3u8Abr203DXR7ZQQfK7fspWjNQ==
